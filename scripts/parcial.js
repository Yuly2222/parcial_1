document.addEventListener('DOMContentLoaded', () => {
    ...
    const sheetURL = 'https://script.google.com/macros/s/AKfycbzcgs7MzdkYAwEfLMuZIil_F3zKYYgM420QsaWTFpGUdOHxb7TzPe1in8m7BJdxuEP5/exec'; // Reemplaza con tu URL de SheetDB o API

    // Cargar productos de forma asincrónica desde Google Sheets
    fetch(sheetURL)
        .then(response => {
            if (!response.ok) {
                throw new Error('Error al cargar los productos desde Google Sheets');
            }
        ...
        .then(products => {
            ...
            products.forEach(product => {
                ...
                productDiv.classList.add('burger', product.categoria.toLowerCase());
                productDiv.setAttribute('data-name', product.nombre);
                productDiv.setAttribute('data-price', parseFloat(product.precio.replace('$', '')));

                productDiv.innerHTML = `
                    <img src="${product.imagen}" alt="${product.nombre}" class="burger-image">
                ...
                burgerOptionsContainer.appendChild(productDiv);
            });

            // Reasignar eventos a los botones "Add to Cart"
    ...
    // Función para asignar eventos a los botones "Add to Cart"
